 ...
 * regardless of whether it uses a bare 'nirnex' or an absolute path like
 * '/usr/local/bin/nirnex'. The setup command now resolves the absolute path
 * at setup time so hook scripts work under /bin/sh's restricted PATH.
 *
 * Pattern: #!/bin/sh\nexec <path-ending-in-nirnex> runtime <subcommand>\n
 */
