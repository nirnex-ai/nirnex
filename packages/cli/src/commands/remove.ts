 ...
 * regardless of whether it uses a bare 'nirnex' or an absolute path, and
 * regardless of whether it contains a PATH export line (added in the PATH
 * injection fix so hooks work in Claude Code's restricted shell environment).
 *
 * Recognised formats (all three must be removable):
 *   Old (no absolute path, no PATH export):
 *     #!/bin/sh
 *     exec nirnex runtime validate
 *
 *   Sprint-28 (absolute path, no PATH export):
 *     #!/bin/sh
 *     exec /usr/local/bin/nirnex runtime validate
 *
 *   Current (absolute path + PATH export — PATH injection fix):
 *     #!/bin/sh
 *     export PATH="/usr/local/bin:/opt/homebrew/bin:$PATH"
 *     exec /usr/local/bin/nirnex runtime validate
 */
function isNirnexHookScriptContent(content: string, subcommand: string): boolean {
  // Optional PATH export line: `export PATH="...":$PATH`
  // Mandatory exec line: `exec <anything ending in nirnex> runtime <subcommand>`
  const pattern = new RegExp(
    `^#!/bin/sh\\n` +
    `(?:export PATH="[^"]*"\\n)?` +       // optional PATH injection line (current format)
