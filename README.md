...
### Layer 2 — Hook scripts use an unresolvable `nirnex` path

Claude Code runs hooks via `/bin/sh` with a restricted PATH (`/usr/bin:/bin`). The `nirnex` binary is installed at `/usr/local/bin/nirnex` — outside that restricted PATH. Every hook silently fails.

**Fix (automatically applied since this version):** `nirnex setup` now calls `resolveNirnexBin()` at setup time to detect the absolute path to the `nirnex` binary, then writes that absolute path into every generated hook script:

```sh
# Generated by nirnex setup ≥ v6.6.0:
#!/bin/sh
exec /usr/local/bin/nirnex runtime validate
```

If you ran `nirnex setup` on an older version, re-run it to regenerate the scripts with the absolute path:

```sh
nirnex remove --keep-data   # removes old hook scripts only
nirnex setup                # regenerates with absolute path
```

Or patch the five scripts manually:

```sh
NIRNEX_BIN=$(which nirnex)
for s in bootstrap entry guard trace validate; do
  printf '#!/bin/sh\nexec %s runtime %s\n' "$NIRNEX_BIN" "$s" \
    > .claude/hooks/nirnex-${s}.sh
  chmod +x .claude/hooks/nirnex-${s}.sh
done
```
